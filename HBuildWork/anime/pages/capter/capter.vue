<template>
	<view class="index">
		...
		<view v-for="(value, index) in data" :key="index" class="capterWrap" @click="preImg(value.id)">
...
<script>
	export default {
		...
		methods: {
			...
			preImg(index) {
				...
				setTimeout(() => {
					uni.navigateTo({
						...
						url: '../capterDetail/capterDetail?data=' + encodeURIComponent(JSON.stringify(index))
					})
					// uni.previewImage({
			...
			getData(e) {
				uni.request({
					url: this.$serverUrl + '/api/detail?list_id=' + e,
					success: (res) => {
