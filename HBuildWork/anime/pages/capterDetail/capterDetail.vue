<template>
	<view class="index">
		<block v-for="(item, index) in data" :key="index">
			<view class="card">
				<image :src="item.url" class="imgPicture"></image>
				<!-- <image class="card-img" :src="item.picture" mode="aspectFill"></image>
				<text class="card-num-view">{{item.type_str}}P</text>
		...
		<!-- <swiper :style="{height:screenHeight + 'px'}">
			<swiper-item v-for="(value,index) in data" :key="value">
				<image :src="value" mode="widthFix"></image>
				<image :src="value.url" class="imgPicture"></image>
				<view>{{ value.chapter }}</view>
			</swiper-item>
		</swiper> -->
		<!-- <cover-image class="capterWrap">
			<view class="capterWrap">{{ value.chapter_name }}</view>
			<image v-for="(value, index) in data" :key="index" :src="value.url" class="imgPicture"></image>
		</cover-image> -->
		<!-- #ifndef H5 -->
...
<script>
	export default {
		...
		methods: {
			...
			getData(e) {
				uni.request({
					url: this.$serverUrl + '/api/queryCapter?chapter_id=' + e,
					success: (res) => {
